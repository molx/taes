...
function atualizaFunc(form) {
    ...
    if (["CD", "CCE"].includes(func)) {
        ...
        $(form).find(".function-percentage-row").css("display", "flex");
    } else {
        $(form).find("input[name='rdCD']").parent().hide();
        $(form).find(".function-percentage-row").hide();
    }
    calcSalario(form);
}
...
function calcSalario(form) {
    ...
    $('form[name="' + form.name + '"] span[name="numJudview"]').css("visibility", "hidden");
    //form.numJudview.style.visibility = "hidden";
    if (form.ddJud.value == 1) {
        $('form[name="' + form.name + '"] span[name="numJudview"]').css("visibility", "visible");
    ...
    var aliqfunp = 0;
    var $funprespPlans = $(form).find(".funpresp-plans");
    var $funprespPlanCards = $funprespPlans.find(".funpresp-plan");
    $funprespPlans.removeClass("has-plan-selection");
    $funprespPlanCards.removeClass("is-selected");
    ...
    if (form.funp_ad.value == "sim") {
        ...
        if (refpss == tetopss && basefunp > 0) {
            ...
            $funprespPlans.addClass("has-plan-selection");
            $funprespPlanCards.eq(0).addClass("is-selected");
            if (form.name == "myform") {
            ...
            $funprespPlans.addClass("has-plan-selection");
            $funprespPlanCards.eq(1).addClass("is-selected");
            if (form.name == "myform") {
        ...
        $(form).find("input[name='funp_plano']").prop("checked", false);
        $(form).find("select[name='ddFunp'], input[name='numFunpAlt']").prop("disabled", true);
    }
    ...
    addDetailValue("#tabdetails-rend", formid, "Vencimento básico", vencimento);
    addDetailValue("#tabdetails-rend", formid, "Auxílio-alimentação", alimentacao);
    if (transporte > 0) addDetailValue("#tabdetails-rend", formid, "Auxílio-transporte", transporte);
    if (creche > 0) addDetailValue("#tabdetails-rend", formid, "Auxílio pré-escolar", creche);
    if (noturno > 0) addDetailValue("#tabdetails-rend", formid, "Adicional noturno", noturno);
    if (jud > 0) addDetailValue("#tabdetails-rend", formid, "Decisão judicial", jud);
    ...
    if (saude > 0) addDetailValue("#tabdetails-rend", formid, "Saúde suplementar", saude);
    if (diffPisoEnf > 0) addDetailValue("#tabdetails-rend", formid, "Diferença do piso da enfermagem", diffPisoEnf);
    if (outrosRendIsnt > 0) addDetailValue("#tabdetails-rend", formid, "Outros rendimentos isentos", outrosRendIsnt);
    if (outrosRendTrib > 0) addDetailValue("#tabdetails-rend", formid, "Outros rendimentos tributáveis", outrosRendTrib);
    if (outrosRendTribPSS > 0) addDetailValue("#tabdetails-rend", formid, "Outros rendimentos tributáveis", outrosRendTribPSS);
    if (abonoperm > 0) addDetailValue("#tabdetails-rend", formid, "Abono de permanência", abonoperm);
    if (ferias > 0) addDetailValue("#tabdetails-rend", formid, "Adicional de um terço de férias", ferias);
    if (adiantamento > 0) addDetailValue("#tabdetails-rend", formid, "Adiantamento", adiantamento);
    if (decter > 0) addDetailValue("#tabdetails-rend", formid, "Décimo terceiro salário", decter);
    //
    ...
    addDetailValue("#tabdetails-desc", formid, "Contribuição previdenciária", valorpss);
    addDetailValue("#tabdetails-desc", formid, "Imposto de renda", aliqirrf);
    if (aliqirrfferias > 0) addDetailValue("#tabdetails-desc", formid, "Imposto de renda sobre férias", aliqirrfferias);
    if (aliqirrfadiant > 0) addDetailValue("#tabdetails-desc", formid, "Imposto de renda sobre adiantamento", aliqirrfadiant);
    if (aliqpssadiant > 0) addDetailValue("#tabdetails-desc", formid, "Contribuição previdenciária do adiantamento", aliqpssadiant);
    if (descAdiant > 0) addDetailValue("#tabdetails-desc", formid, "Adiantamento", descAdiant);
    if (desc_13 > 0) addDetailValue("#tabdetails-desc", formid, "Imposto de renda e contribuição previdenciária do décimo terceiro", desc_13);
    if (aliqfunp > 0) addDetailValue("#tabdetails-desc", formid, "Funpresp", aliqfunp);
    if (aliqFunpFacul > 0) addDetailValue("#tabdetails-desc", formid, "Contribuição facultativa adicional à Funpresp", aliqFunpFacul);
    if (sindicato > 0) addDetailValue("#tabdetails-desc", formid, "Sindicato", sindicato);
    if (outrosdescontos > 0) addDetailValue("#tabdetails-desc", formid, "Outros descontos tributáveis", outrosdescontos);
    if (outrosdescontospct > 0) addDetailValue("#tabdetails-desc", formid, "Outros descontos tributáveis", outrosdescontospct);
    if (outrosdescontosIsnt > 0) addDetailValue("#tabdetails-desc", formid, "Outros descontos isentos", outrosdescontosIsnt);
    if (outrosdescontospctIsnt > 0) addDetailValue("#tabdetails-desc", formid, "Outros descontos isentos", outrosdescontospctIsnt);

    addDetailValue("#tabdetails-outros", formid, "Rendimento bruto", bruto);
    addDetailValue("#tabdetails-outros", formid, "Total de descontos", descontos);
    addDetailValue("#tabdetails-outros", formid, "Rendimento líquido", salario);
    addDetailValue("#tabdetails-outros", formid, "Base da contribuição previdenciária", basepss);
    addDetailValue("#tabdetails-outros", formid, "Rendimentos tributáveis", rendTributavel);
    addDetailValue("#tabdetails-outros", formid, "Deduções do imposto de renda", Math.max(deducoesIrrf, deducaoSimp));
    addDetailValue("#tabdetails-outros", formid, "Base do imposto de renda", baseirrf);
    addDetailValue("#tabdetails-outros", formid, "Desconto do imposto de renda", descIrrf);
